...
public class QServiceCfg {
    ...
    public QServiceCfg(Context context){
        ...
        // 如果要接入CVM进行鉴权，请使用
        // cosXmlService = new CosXmlService(context, cosXmlServiceConfig, new MyCVMCredentialProvider());
    }
    ...
    private class MyCredentialProvider extends CosXmlCredentialProvider{
        ...
        public BasicSignaturePair signaturePair() throws QCloudException {
            ...
            return new BasicSignaturePair(secretKeyToSignKey(secretKey, keyTime), keyTime);
        }
    ...
    private class SafeCredentialProvider extends CosXmlCredentialProvider{
        ...
        public BasicSignaturePair signaturePair() throws QCloudException {
            ...
            return new BasicSignaturePair(signKey, keyTime);
        }
    ...
    /**
     *
     * 通过接入CAM进行鉴权，需要用户后台向CAM请求临时秘钥和token。临时秘钥过期后，SDK会自动回调该方法获取新的临时秘钥
     *
     * SessionCredential 有四个参数：
     *    tempSecretId : 临时secretId
     *    tempSecretKey : 临时secretKey
     *    token : token
     *    expire : 过期时间 (timestamp)
     *
     * 请直接使用CAM返回的对应四个字段   
     *
     */
    class MyCVMCredentialProvider extends SmartSessionCredentialProvider {
        ...
        protected SessionCredential sessionCredential() throws QCloudException {
            ...
            // TODO: 17/9/4  向CAM后台请求临时秘钥SessionCredential
